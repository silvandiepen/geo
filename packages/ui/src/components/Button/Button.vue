	...
	const hasMeaningfulNode = (nodes: VNodeArrayChildren): boolean => {
		...
		return nodes.some((node) => {
			...
			if (vnode.type === Fragment) {
				return hasMeaningfulNode(vnode.children as VNodeArrayChildren);
			}
...
const showText = computed(() => {
	return isLoading.value || hasText.value;
});
...
const normalizedTo = computed<RouteLocationRaw | null>(() => {
	...
	if (typeof props.to !== 'object' || Array.isArray(props.to)) {
		return props.to;
	}

	const routeTarget = Object.assign(
		{},
		props.to as Record<string, unknown>
	) as ButtonRouteLocation;
...
.sil-button {
	...
	--int-button-color: var(
		--prop-button-color,
		var(--button-color, var(--color-secondary))
	);
	...
	--int-button-text: var(
		--prop-button-text,
		var(--button-text, var(--color-secondary-contrast))
	);

	--int-button-color--hover: color-mix(
		in srgb,
		var(--int-button-color),
		var(--color-background) 10%
	);

	--int-button-background-color--hover: color-mix(
		in srgb,
		var(--int-button-background-color),
		var(--color-background) 50%
	);

	--int-button-text--hover: color-mix(
		in srgb,
		var(--int-button-text),
		var(--color-background) 10%
	);
	...
	--int-button-icon-color--hover: var(
		--button-icon-color--hover,
		var(--int-button-icon-color)
	);
	...
	border-radius: var(
		--int-button-border-radius,
		var(--button-border-radius, var(--border-radius-xl))
	);
	...
	&__container {
		...
		padding: var(
			--int-button-padding,
			var(--button-padding, var(--space-s) var(--space-m))
		);
	...
	&--ghost {
		...
		--int-button-text: color-mix(
			in srgb,
			var(--int-button-color),
			var(--color-foreground) 33.33%
		);
		--int-button-text--hover: color-mix(
			in srgb,
			var(--int-button-text),
		...
		--int-button-icon--hover: color-mix(
			in srgb,
			var(--int-button-text),
	...
	&--naked {
		--int-button-background-color: var(
			--button-background-color,
			color-mix(in srgb, var(--int-button-color), transparent 100%)
		);
		--int-button-background-color--hover: var(
			--button-background-color--hover,
			color-mix(in srgb, var(--int-button-color), transparent 75%)
		);
		--int-button-border-color: var(
			--button-border-color,
			var(--int-button-color)
		);
